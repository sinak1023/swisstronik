...
<div class="max-w-7xl mx-auto p-4 md:p-6">
    ...
    <div class="bg-surface rounded-2xl shadow-sm border p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                <label class="block text-sm mb-2">سابقهٔ کمپین</label>
                <select id="search_duplicates" class="w-full px-4 py-2 border rounded-lg bg-background">
                    <option value="">همه</option>
                    <option value="1">فقط لیدهای سابقه‌دار (تکراری از کمپین قبلی)</option>
                </select>
            </div>
            <div>
    ...
    <div class="bg-surface rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-3 px-6 py-4 border-t">
            <div class="flex items-center gap-3">
                ...
                <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                    نمایش
                    <select id="perPageSelect" class="px-2 py-1.5 border rounded-lg bg-background text-text text-sm">
...
<!-- مودال جزئیات لید (فقط‌خواندنی برای مدیر): مشخصات + یادداشت‌ها + پرداخت‌ها + جلسات -->
<div id="leadDetailModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" style="z-index: 1350;">
    <div class="bg-surface rounded-2xl shadow-2xl border border-border w-full max-w-3xl max-h-[88vh] flex flex-col overflow-hidden">
        <div class="sticky top-0 bg-surface border-b border-border px-6 py-4 flex justify-between items-center">
            <h3 class="text-xl font-bold text-text">مشخصات کامل لید</h3>
...
<script>
    ...
    // تعداد لید در هر صفحه (قابل انتخاب توسط کاربر، ذخیره در مرورگر)
    const PER_PAGE_OPTIONS = [25, 50, 100, 200, 500];
    let pageLimit = parseInt(localStorage.getItem('leads_per_page'), 10);
    if (PER_PAGE_OPTIONS.indexOf(pageLimit) === -1) pageLimit = 100;
    ...
    function renderTable(leads) {
        ...
        // با هر بارگذاری مجدد جدول، انتخاب‌های قبلی معتبر نیستند
        selectedLeads = [];
        lastChecked = null;
        ...
        leads.forEach(lead => {
            ...
            <tr class="hover:bg-muted/30 transition-colors">
                ...
                <td class="px-6 py-4">
                    ...
                    ${(lead.history_count && lead.history_count > 0) ? `<span class="inline-flex items-center gap-1 mr-1 text-[10px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full" title="این شماره در ${lead.history_count} کمپین دیگر هم سابقه دارد"><i class='bx bx-history'></i> سابقه‌دار</span>` : ''}
                </td>
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center justify-center gap-3">
                        ...
                        <button onclick="openLeadDetail(${lead.id})" class="text-emerald-600 hover:underline text-sm">
                            <i class='bx bx-detail'></i> مدیریت
                        </button>
                    </div>
                </td>
                <td class="px-6 py-4">
    ...
    $('#searchBtn').on('click', () => {
        const filters = {
            ...
            only_duplicates: $('#search_duplicates').val(),
    ...
    function renderPagination(page, pages, total) {
        ...
        // پنجره‌ای از صفحات اطراف صفحهٔ جاری
    ...
    // مقداردهی امن select2 برای سلکت کارشناس؛ اگر select2 لود نشده باشد،
    // سلکت معمولی مرورگر کار می‌کند و مودال هرگز از باز شدن نمی‌افتد.
    function initAssignSelect(multiple) {
        const $sel = $('#assign_user');
        try {
            if (typeof $.fn.select2 === 'function' && $sel.hasClass('select2-hidden-accessible')) {
                $sel.select2('destroy');
            }
        } catch (err) {
            console.error('select2 destroy failed:', err);
        }
        $sel.prop('multiple', !!multiple).prop('disabled', false).val(multiple ? [] : '');
    ...
    function showAssignModal() {
        ...
        initAssignSelect($('#roundRobin').is(':checked'));
    }

    $('#roundRobin').on('change', function() {
        initAssignSelect(this.checked);
    });
    ...
    function renderImportPreview(data) {
        ...
        // فعال کردن Select2 (در صورت لود نشدن پلاگین، سلکت معمولی کار می‌کند)
        setTimeout(() => {
            if (typeof $.fn.select2 !== 'function') return;
            try {
                $('.select4').select2({
    ...
    /* ===== مودال جزئیات لید (فقط‌خواندنی) ===== */
    function meetingStatusMetaLD(status) {
        const map = {
            'success':     { chip: 'bg-green-100 text-green-800', label: 'موفق' },
            'failed':      { chip: 'bg-red-100 text-red-800', label: 'ناموفق' },
            'rescheduled': { chip: 'bg-yellow-100 text-yellow-800', label: 'جلسه مجدد' },
            'scheduled':   { chip: 'bg-gray-100 text-gray-700', label: 'زمان‌بندی‌شده' },
        };
        return map[status] || map['scheduled'];
    }

    function openLeadDetail(leadId) {
        if (!leadId) { showSnackbar('لید نامعتبر', 'error'); return; }
    ...
    function renderLeadDetail(res) {
        ...
        if (res.meetings.length) {
            ...
            res.meetings.forEach(m => {
                const meta = meetingStatusMetaLD(m.status);
                html += `
    ...
    $(document).ready(() => {
        $('#perPageSelect').val(String(pageLimit)).on('change', function() {
            const val = parseInt(this.value, 10);
            pageLimit = PER_PAGE_OPTIONS.indexOf(val) === -1 ? 100 : val;
            localStorage.setItem('leads_per_page', pageLimit);
...
<script>
    if (window.jQuery && typeof $.fn.select2 === "function") {
        try { $(".select2").select2(); } catch (e) { console.error(e); }
    }
</script>
    ';
?>