...
class Leads
{
    ...
    {
        // has_history: آیا همین شماره (نرمال‌شده) در پروژه/کمپین دیگری هم وجود دارد
        $sql = "SELECT l.*, p.name as project_name, u.name as assignee_name,
                       (SELECT COUNT(*) FROM {$this->table} l2
                        WHERE l2.phone_norm = l.phone_norm
                          AND l2.phone_norm IS NOT NULL AND l2.phone_norm <> ''
                          AND l2.project_id <> l.project_id) AS history_count
                FROM {$this->table} l 
                LEFT JOIN projects p ON l.project_id = p.id 
                LEFT JOIN users u ON l.assigned_to = u.id 
                WHERE 1=1";
        ...
        // فیلتر: فقط لیدهایی که سابقهٔ کمپین قبلی دارند
        if (!empty($filters['only_duplicates'])) {
            $sql .= " AND EXISTS (SELECT 1 FROM {$this->table} l3
                        WHERE l3.phone_norm = l.phone_norm
                          AND l3.phone_norm IS NOT NULL AND l3.phone_norm <> ''
                          AND l3.project_id <> l.project_id)";
        }
    ...
    /**
     * یافتن لیدهای «اصلی» متعلق به کارشناسِ دیگر که شمارهٔ اصلی‌شان با شمارهٔ
     * داده‌شده (نرمال‌شده) یکی است. برای هشدار «شمارهٔ ثانویه».
     * لیدهای بدون کارشناس یا متعلق به همان کارشناس نادیده گرفته می‌شوند.
     */
    public function find_primary_owned_by_others($phone, $exclude_lead_id, $exclude_user_id)
    {
        $norm = Phone::normalize($phone);
        if ($norm === '') return [];
        $sql = "SELECT l.id, l.name, l.phone, l.project_id, l.assigned_to,
                       p.name AS project_name, u.name AS assignee_name
                FROM {$this->table} l
                LEFT JOIN projects p ON l.project_id = p.id
                LEFT JOIN users u ON l.assigned_to = u.id
        ...
        return $this->db->fetchAll($sql, [$norm, (int)$exclude_lead_id, (int)$exclude_user_id]);
    }
