...
// ===== بررسی تداخل شمارهٔ ثانویه با لید اصلیِ کارشناس دیگر =====
// اگر شماره‌ای ثانویه (تلگرام/واتساپ/بله) وارد شده که شمارهٔ اصلیِ لیدِ کارشناس
// دیگری است، هشدار ثبت و یک‌بار به مدیر نوتیف داده می‌شود.
try {
    $secondaries = [];
    if ($telegram_phone !== '') $secondaries['telegram'] = $telegram_phone;
    if ($whatsapp_phone !== '') $secondaries['whatsapp'] = $whatsapp_phone;
    if ($bale_phone !== '')     $secondaries['bale']     = $bale_phone;

    if (!empty($secondaries)) {
        (new SecondaryAlerts($db))->checkAndRegister(
            ['id' => $id, 'assigned_to' => $lead['assigned_to']],
            $secondaries
        );
    }
} catch (Exception $e) {
    error_log('secondary alert check error: ' . $e->getMessage());
}
